...
> **Nota:** il progetto è impostato per usare Node.js 20 (file `.node-version` dentro `backend/`). Se Render usa una versione di Node troppo recente, il modulo del database (`better-sqlite3`) può andare in crash all'avvio — è già risolto in questo progetto, non serve fare nulla, ma se in futuro aggiorni le dipendenze e vedi un crash simile a `Assertion failed... better_sqlite3.node`, è sempre questo il motivo.
